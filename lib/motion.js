...
MotionDetection.prototype.getBlendedImage = function(img1, img2) {
  ...
  var i = 0;
  while (i < (parseInt(img1.length/3))) {    
    var average1 = (img1[3 * i] + img1[3 * i + 1] + img1[3 * i + 2]) / 3;
    var average2 = (img2[3 * i] + img2[3 * i + 1] + img2[3 * i + 2]) / 3;
...
MotionDetection.prototype.diff = function(img1, img2) {
  ...
  return Math.round(changed / (img1.length/3));
};
